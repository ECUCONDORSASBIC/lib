...
module.exports = function (source) {
  ...
  // Use a hardcoded relative path to the polyfill instead of require.resolve
  const polyfillPath = '../polyfills/async-hooks-polyfill.js';

  // Replace all node:async_hooks imports with our polyfill using relative paths
  const modifiedSource = source.replace(
    /require\(['"]node:async_hooks['"]\)/g,
    "require('" + polyfillPath + "')"
  ).replace(
    /import\s+.*\s+from\s+['"]node:async_hooks['"]/g,
    "import AsyncHooks from '" + polyfillPath + "'"
  ).replace(
    /import\s+{(.*)}\s+from\s+['"]node:async_hooks['"]/g,
    "import {$1} from '" + polyfillPath + "'"
  ).replace(
    /import\s+['"]node:async_hooks['"]/g,
    "import '" + polyfillPath + "'"
  );
